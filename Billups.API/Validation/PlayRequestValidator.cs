...
{
    public PlayRequestValidator(ICurrentChoiceResolver  currentChoiceResolver)
    {
        var validChoiceIds = currentChoiceResolver.GetAll().Select(c => c.Id).ToList(); // if too expensive, the list could be cached
        RuleFor(x => x.Player)
